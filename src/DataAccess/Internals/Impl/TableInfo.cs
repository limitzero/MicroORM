...
namespace MicroORM.DataAccess.Internals.Impl
{
	public class TableInfo
	{
		...
		public ICollection<string> GetFieldsForSelect()
		{
			...
			fields.Add(this.PrimaryKey.DataColumnName);
			...
			foreach (var column in this.References)
			{
				...
				if (primarykey != null)
				{
					...
					{
						fields.Add(primarykey.DataColumnName);
					}
				}
			}
		...
		{
			return MicroORM.Configuration.Instance.Dialect.BuildSelectStatement(this.TableName,
			                                                                    this.GetFieldsForSelect());
		}

		public string GetInsertStatement(object entity)
		{
			return MicroORM.Configuration.Instance.Dialect
				.BuildInsertStatement(this.TableName,
				                      this.GetFieldsForUpsert(entity));

			//var builder = new StringBuilder();
			//var statement = string.Format("INSERT INTO [{0}] ", this.TableName);
			//builder.Append(statement);
			//builder.Append(this.BuildFieldsForInsert(entity));
			//builder.Append(string.Concat(this.BuildValuesClauseForInsert(entity), ";"));
			//builder.Append("SELECT CAST(ISNULL(SCOPE_IDENTITY(), 0) AS INT) AS ID");
		...
		{
			return MicroORM.Configuration.Instance.Dialect
				.BuildDeleteStatement(this.TableName,
				                      this.PrimaryKey.DataColumnName);
		...
		{
			return MicroORM.Configuration.Instance.Dialect
				.BuildUpdateStatement(this.TableName,
				                      this.GetFieldsForUpsert(entity),
				                      this.PrimaryKey.DataColumnName);

			//var builder = new StringBuilder();
			//var statement = string.Format("UPDATE [{0}] SET ", this.TableName);
			//builder.Append(statement);
			//builder.Append(this.GetColumnsWithParameterAssignment(entity));
			//return builder.ToString();
		}
		...
		public string BuildValueAssignmentForUpdate(object entity)
		{
			...
			{
				if (columnInfo == this.PrimaryKey || columnInfo.Column.PropertyType.IsGenericType) continue;
		...
		public static bool IsPropertyPopulated(ColumnInfo columnInfo, object entity)
		{
			...
			if (columnInfo.Column.CanRead)
			{
				...
				if (property != null)
				{
					...
					if (propertyValue == null) return isPopulated;
