...
{
	public class MaxOnFieldFromEntitySelectOption<TEntity> : ISelectOption
	{
		...
		public MaxOnFieldFromEntitySelectOption(Expression<Func<TEntity, object>> field, string columnName)
		{
		...
		public void Build()
		{
			if (this.MetadataStore.Entities.ContainsKey(typeof (TEntity)))
			{
				...
				string targetField = string.Format("{0} = max({1}.{2})", this.columnName, table, datacolumn);
